# Test Quality：默认原则（Default Principles）

这些默认值综合了 Gerard Meszaros 的 xUnit Test Patterns、Kent Beck 的 TDD、Martin Fowler 的测试指导为一个可操作的集合。

如果项目有自定义 `.lattice/test-quality.md`（通过 `.lattice/config.yaml` 引用），该文档优先。

## 目录（Table of Contents）

1. [AAA 结构](#1-aaa-结构)
2. [每个测试一个行为](#2-每个测试一个行为)
3. [断言模式](#3-断言模式)
4. [测试隔离技术](#4-测试隔离技术)
5. [测试命名约定](#5-测试命名约定)
6. [测试数据构建器和工厂](#6-测试数据构建器和工厂)
7. [测试金字塔分布](#7-测试金字塔分布)
8. [反模式目录](#8-反模式目录)

---

## 1. AAA 结构（AAA Structure）

每个测试：三个清晰阶段。读者一眼就能识别每个阶段。

### 良好的分离（Good Separation）
...
### 当 Arrange 很长时（When Arrange Is Long）
...
## 2. 每个测试一个行为（One Behavior Per Test）

### 拆分多行为测试（Splitting a Multi-Behavior Test）
...
### 多个断言对于单一行为是可以的（Multiple Assertions Are Fine for One Behavior）
...
## 3. 断言模式（Assertion Patterns）

### 具体 vs 通用（Specific vs Generic）
...
### 自定义断言辅助函数（Custom Assertion Helpers）
...
### 对"未发生"的断言（Asserting on the Negative Space）
...
## 4. 测试隔离技术（Test Isolation Techniques）
...
## 5. 测试命名约定（Test Naming Conventions）

### 模式：should_[行为]_when_[条件]（Pattern: should_[behavior]_when_[condition]）
...
### 模式：[方法]_[场景]_[预期结果]（Pattern: [method]_[scenario]_[expected]）
...
### 反模式名称（避免）（Anti-Pattern Names (Avoid)）
...
### 语言特定约定（Language-Specific Conventions）
...
## 6. 测试数据构建器和工厂（Test Data Builders and Factories）
...
### 工厂函数（更简单的替代方案）（Factory Functions (Simpler Alternative)）
...
### 边界值的命名常量（Named Constants for Boundary Values）
...
## 7. 测试金字塔分布（Test Pyramid Distribution）

### 指导比例（Guideline Ratios）
...
### 何时编写每种类型（When to Write Each Type）
...
### 向下推动覆盖率（Push Coverage Downward）
...
## 8. 反模式目录（Anti-Pattern Catalog）

### Test-per-Method（每个方法一个测试）
