...
- 部署开源模型、开源编码智能体以及搭配扩展需要投入时间，但回报是透彻的技术理解，以及一个稳定的基座，让后续研发投入形成复利效应。
...
35B 是一个混合专家模型。
虽然这类模型占用更多内存，但在 Mac 或像我使用的 Framework 笔记本这类无法将整个模型放入 GPU 的设备上，它们运行起来要快得多 —— 因为每个 token 只有 30 亿参数处于激活状态，而 “稠密” 模型则需要激活 270 亿参数。
稠密模型在连贯性方面可能更好一些。
...
基准测试结果是在我开始使用之后才发布的。
尝试摸索有尝试摸索的时候，制作小工具有制作小工具的时候，之后我还会再回来继续摸索的。
