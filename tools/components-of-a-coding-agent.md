...
---
在本文中，我将介绍编码智能体 (coding agent) 与智能体工具链 (agent harness) 的整体设计：它们是什么、如何运作，以及各部分在实际应用中如何协同工作。
...
*图 2：传统 LLM、推理 LLM（或称推理模型），以及封装在智能体工具链中的 LLM 之间的关系。*
...
- 智能体工具链 (agent harness)：包裹在智能体外的软件框架 (scaffold)，用于管理上下文、工具调用、提示词、状态与控制流
...
（严格来说，智能体是由模型驱动的决策循环，而工具链是提供上下文、工具与执行支持的外围软件架构 (scaffold)。）
...
“智能” 的运行时 (runtime) 不会在每一轮都将所有内容重新构建为一个巨大的无差别提示词，如下图所示。
...
<ins>因此，当模型要求做某事时，运行时 (runtime) 可以停下来并执行程序化检查，例如</ins>：
