...
到目前为止，我已经讨论了关于 *限界上下文 (Bounded Context)* 、*子域 (Subdomain)* 和 *上下文映射 (Context Mapping)* 的战略设计。这里你看到两个 *限界上下文 (Bounded Context)* ，名为 *敏捷项目管理上下文 (Agile Project Management Context)* 的 *核心域 (Core Domain)* 和一个 *支撑子域 (Supporting Subdomain)* ，其通过 *上下文映射 (Context Mapping)* 集成提供协作工具。

<center><img src="../imgs/fig-81.png" width="100%"/></center><br/>

但是，那些 *限界上下文 (Bounded Context)* 内部存在的概念呢？
...
<ins>每个 *聚合 (Aggregate)* 形成一个事务一致性边界</ins>。
...
但你应该首先关注事务一致性 (transactional consistency)。
...
<ins>事务边界的理由是业务驱动的，因为在任意给定时间，业务决定了集群应该处于何种有效状态</ins>。
...
<ins>任何其他的 *聚合 (Aggregate)* 将在单独的事务中被修改和提交。
这就是为什么 *聚合 (Aggregate)* 被称为事务一致性边界</ins>。
...
<ins>从本节要记住的主要一点是，业务规则是确定在单个事务结束时，什么必须是完整的、完备的和一致的驱动因素</ins>。
