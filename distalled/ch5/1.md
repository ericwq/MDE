...
规则 1 意味着，业务最终应基于事务提交时必须保持一致的内容来确定 *聚合 (Aggregate)* 组合。
...
这条规则强调，每个 *聚合 (Aggregate)* 的内存占用和事务范围应该相对较小。
...
在设计 *聚合 (Aggregates)* 时，另一件需要记住的事情是 *单一职责原则（SRP）* 。
...
*领域事件 (Domain Event)* 由 *聚合 (Aggregate)* 发布，并由感兴趣的 *限界上下文 (Bounded Contxt)* 订阅。
消息传递机制通过订阅将 *领域事件 (Domain Event)* 传递给相关方。
