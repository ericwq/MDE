...
持续交付 (continuous delivery) ——一种自动确保你的软件随时可以发布到生产环境的实践—— 可以帮助你做到这一点。
通过持续交付，你使用构建流水线 (build pipeline) 来自动测试你的软件并将其部署到测试和生产环境。
...
- <ins>编写不同粒度 (granularity) 的测试</ins>
...
即使从头开始编写自己的 Mock，也只是编写一个与真实签名相同的伪造的类/模块/函数，并在测试中设置它。
...
Spring Data 分析该方法的返回类型及其方法名，并根据命名约定检查方法名以确定它应该做什么。
