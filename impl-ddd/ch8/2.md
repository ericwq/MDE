...
若订阅方需要的信息不仅限于事件的触发原因，那么就可能需要为事件补充额外的状态和行为。这些信息可通过增强状态（更多 properties）或提供能派生更多状态的操作来传递。这样一来，订阅方便无需回查发布该事件的`Aggregate`，避免了可能产生的不必要的操作难度或性能开销。在采用`Event Sourcing`时，事件增强的做法会更为常见，因为用于持久化的事件在跨`Bounded Context`发布时，往往需要包含更多状态信息。有关事件增强的示例可参见 [附录 A](../appendix.md) 。
