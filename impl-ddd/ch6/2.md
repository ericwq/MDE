## 与极简主义结合（Integrate with Minimalism）

#### ▶[上一节](1.md)

在每一个 DDD 项目中，总会存在多个`Bounded Contexts`，这意味着我们必须找到合适的方式来集成它们。<ins>当来自上游上下文的对象流入下游上下文时，只要有可能，就应该使用`Value Objects`来为下游上下文中的概念建模。</ins>

<ins>通过这种方式，你可以以极简主义为优先原则进行集成，也就是说，尽量减少你在下游模型中需要负责管理的属性 (properties) 数量。使用不可变`Values`会让你承担更少的责任。</ins>
