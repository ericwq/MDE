...
如果状态为 REQUESTED，一个长时运行进程（Long-Running Process）会负责管理协作讨论的创建，以及该讨论后续与 Product 的关联初始化。
...
如果讨论可用性的状态为 REQUESTED，事件构造函数的最后一个参数将为 true，这正是启动长时运行进程所需的条件。
...
我们需要创建一个简单的长时运行进程，来管理讨论的创建，然后将其关联到 Product 。

在深入长时运行进程的细节之前，我们再考虑一种请求讨论的可能方式。
...
同样，如果讨论可用性的状态为 REQUESTED，事件构造函数的最后一个参数将为 true，这正是启动长时运行进程所需的条件：
...
因此，可能需要进行一些干预。例如，进程可能需要向管理员组发送一封电子邮件。

在`Agile Proj- ect Management Context`一侧，用于管理长时运行 进程的类，与用于管理 ProductOwner 和 TeamMember `Aggregates`的创建与维护的类类似（参见上一节）。
...
这就是这个长时运行进程（Long-Running Process）的核心内容。
...
不过，还有另一个因素需要考虑，它要求我们更谨慎地管理长时运行进程的成功执行。
...
我们必须假定，如果当前状态为 READY，则长时运行进程已经完成。
