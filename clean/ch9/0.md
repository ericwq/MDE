...
> 这里需要的是类似下面这样的替换性质：如果对于类型 S 的每一个对象 o1，都存在一个类型 T 的对象 o2，使得对于所有以 T 方式定义的程序 P，当 o1 替换掉 o2 时，P 的行为保持不变，那么 S 就是 T 的子类型。<sup>[1](#1)</sup>

为了理解这个被称为里氏替换原则（`LSP`）的思想，让我们来看一些例子。
...
从架构角度理解 `LSP` 的最佳方式是观察当一个系统违反该原则时，其架构会发生什么。
