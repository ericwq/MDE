...
这类替代方案和设计权衡无处不在，仅在这个简化的系统中我就能列举大量例子。但关键在于，这些都是同一模型内的自由度。通过将`VALUES`、 [ENTITIES](../ch5/2.md) 及其`AGGREGATES`进行建模，我们已有效降低了此类设计变更的影响。例如在此案例中，所有变更都被封装在`Cargo`的 [AGGREGATE](../glossary.md#aggregate) 边界内。虽然需要新增`Handling Event Repository`，但无需重构`Handling Event`本身（尽管具体实现可能涉及调整，这取决于 [REPOSITORY](../glossary.md#repository) 框架的细节）。
