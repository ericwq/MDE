...
`Delivery History`反映`Cargo`实际经历的状态，而`Delivery Specification`则描述预期目标。<ins>`Delivery History`对象可通过分析最近的装卸操作及对应`Carrier Movement`的目的地，计算出`Cargo`的当前`Location`。成功的交付将以满足`Delivery Specification`目标的`Delivery History`为终结。</ins>
