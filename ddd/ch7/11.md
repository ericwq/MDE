...
若需其他集成（例如使用`Sales Management System`客户数据库替代我们自身的`Customer` [REPOSITORY](../glossary.md#repository) ），可创建另一个转换器，承担该职责的 [SERVICES](../ch5/4.md) 。此时仍可设置`Sales Management System Interface`这类底层类，来处理与外部程序的通信机制，但该类不负责数据转换。此外，该接口将隐藏在`Allocation Checker`后面，因此不会出现在领域设计中。
