...
本章第一个示例描述了一个负责货物路由的航运系统。这种路由流程具有业务意义。[SERVICE](../ch5/4.md) 正是将此类流程显式表达的一种方式，同时仍能封装极其复杂的算法。
...
假设存在一个应用需求：列出所有有逾期`Invoices`的客户。理论上，我们先前定义的`Delinquent Invoice Specification`仍可适用，但实际实现可能需要调整。为证明 *概念* 是相同的，首先假设`Invoices`数量较少，可能已全部加载至内存。此时，为验证目的开发的简单实现依然有效。`Invoice Repository`可提供通用方法，根据`SPECIFICATION`筛选`Invoices`：
...
现在，如果我们将这些写成`Container Specifications`，我们应该能够拿一个准备好的 (packed) 容器配置来测试，看它是否符合这些约束条件。
...
然而， *管理 (governing)* 打包的规则尚未从领域对象中抽离。
