...
<img src="../img/fig9.11b.png" width="80%" />
...
当基础设施无法提供帮助时，我们可通过在`Invoice Repository`中添加专用查询方法，将 SQL 从表达性领域对象中重构出来。为避免将规则嵌入 [REPOSITORY](../ch6/3.md) ，我们必须采用更通用的查询表达方式，这种方式不直接捕获规则，但可通过组合或置于上下文中来实现规则（本例中通过双重分派实现）。
```java
...
我们可以先写一个程序，把一种化学品放入容器中，但我们先从验证问题开始吧。这将迫使我们把规则明确化，并为最终实现提供一种测试方法。
...
现在，如果我们将这些写成`Container Specifications`，我们应该能够拿一组已打包的容器配置来测试它是否符合这些约束条件。
...
对领域的理解和基于`SPECIFICATION`的模型，我们得以定义清晰简洁的 [SERVICE](../ch5/4.md) 接口：该接口将接收`Drums`集合与`Containers`集合，并按规则完成打包。
```java
...
```
现在，设计一个优化约束求解器以履行`Packer`服务职责的任务，已与应用程序其余部分解耦，这些机制不会干扰表达模型的设计部分。（参见[第 10 章](../ch10/0.md) 中的 [声明式设计风格](../ch10/7.md) 及 [第 15 章](../ch15/0.md) 中的 [COHESIVE MECHANISM](../ch15/6.md) 。） 然而， *管理* 打包的规则尚未从领域对象中抽离。 *（译注：这个例子看了两遍都没看懂，TODO 再看看其他版本）*
...
*（译注：这两个例子说明的问题似乎和`SPECIFICATION`关联度不大。TODO 再多看看，看能否理解这部分的内容）*
