...
[第 8 章](../ch8/0.md) 讲述了一个关于建立银团贷款系统项目中实现模型突破的故事。
现在将详细说明这个例子，只聚焦于与该项目设计类似的某一项功能展开说明。
...
让我们再进一步把它变成 [CLOSURE OF OPERATIONS](6.md) 。
与其 “增加” 一个`Share Pie`或向其中添加`Shares`，不如直接把两个`Share Pies`加在一起：结果就是新的、更大的`Share Pie`。
...
我们可以对新的 [VALUE OBJECTS](../ch5/3.md) `Share Pies`做出一些明确的 [ASSERTIONS](3.md) 。每个方法都具有特定含义。 *（译注：断言在下面的代码中以 ASSERTIONS 开头的注释行表示）*
...
这些简短方法各自阐明了其 *含义* 。
应用本金支付意味着从贷款中扣除该笔款项。
分配本金支付则是将金额 *按比例* 分摊给各股东。
`Share Pie`的设计使我们在`Loan`代码中得以采用声明式风格，生成的代码开始呈现出商业交易的概念性定义特征，而非单纯的计算逻辑。
...
灵活的设计对软件应对变化与复杂性的能力具有深远影响。
正如本章示例所示，其成败往往取决于相当精细的建模与设计决策。
这种影响可能超越特定建模与设计问题范畴。
[第 15 章](../ch15/0.md) 将讨论灵活设计的战略价值，将其作为提炼领域模型、让大型复杂项目更易于掌控的若干工具之一。
