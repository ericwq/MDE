...
我们可以对新的 [VALUE OBJECTS](../ch5/3.md) ，`Share Pies`，做出一些明确的`ASSERTIONS`。每个方法都具有特定含义。
```java
...
- **复杂逻辑被封装在专用的 [VALUE OBJECTS](../ch5/3.md) 中，这些对象使用`SIDE-EFFECT-FREE FUNCTIONS`实现。** 绝大多数复杂逻辑已被封装在这些不可变对象中。由于`Share Pies`属于 [VALUE OBJECTS](../ch5/3.md) ，数学运算可创建新实例，我们可自由使用这些新实例替换过时的实例。
