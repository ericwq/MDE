...
其次，通常存在替代模型和设计方案，完全无需修改现有对象。取而代之的是创建并返回一个新的 [VALUE OBJECT](../ch5/3.md) ，该对象代表计算结果。这是常见的技术手段，后续示例将予以说明。[VALUE OBJECT](../ch5/3.md) 可在响应查询时创建，完成传递后即可被遗忘——这与生命周期受严格管控的 [ENTITY](../ch5/2.md) 截然不同。

[VALUE OBJECTS](../ch5/3.md) 是不可变的，这意味着除创建时调用的初始化器外，其所有操作均为函数。如同函数，[VALUE OBJECTS](../ch5/3.md) 更安全且易于测试。将逻辑或计算与状态变更混合的操作应重构为两个独立操作（ [Fowler 1999](../references.md#fowler-1999) , p. 279）。但根据定义，这种将副作用分离为简单命令方法的做法仅适用于 [ENTITIES](../ch5/2.md) 。在完成修改与查询分离的重构后，可考虑二次重构：将复杂计算的责任移交至 [VALUE OBJECT](../ch5/3.md) 。通过创建 [VALUE OBJECT](../ch5/3.md) `值对象`替代状态变更，或将全部责任移交至 [VALUE OBJECT](../ch5/3.md) ，通常能彻底消除副作用。
...
[SIDE-EFFECT-FREE FUNCTIONS](../glossary.md#side-effect-free-function) ，尤其是在不可变的 [VALUE OBJECTS](../ch5/3.md) 中，允许安全地组合操作。当一个`FUNCTION`通过 [INTENTION-REVEALING INTERFACE](1.md) 呈现时，开发者可以在不了解其具体实现的情况下使用它。
...
将 `Pigment Color` 抽取出来确实比之前的版本传达了更多信息，但计算方式还是相同的，仍然在 `mixIn()` 方法中。当我们把颜色数据移出来时，也应该把相关的行为一起带出来。在执行之前，需要注意的是 `Pigment Color` 是一个 [VALUE OBJECT](../ch5/3.md) 。因此，它应该被视为不可变的。当我们调配油漆时，`Paint` 对象本身会发生变化。它是一个有持续生命周期的 [ENTITY](../ch5/2.md) 。相比之下，表示特定黄色的 `Pigment Color` 总是恰好就是那种颜色。混合操作会产生一个新的 `Pigment Color` 对象，代表新的颜色。
