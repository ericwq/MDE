...
它支持对基础方面 (fundamental aspects) 的修改，例如类型结构和函数调用机制。
在该模式中，应用程序被划分为两部分：
...
针对该问题的许多解决方案，如实现类型特定的存储和读取方法，不仅成本高昂，而且容易出错。
...
该类结构的变更要求我们修改现有应用程序类中的这些方法。
持久化与应用功能紧密耦合。
*「不能因为持久化功能而导致应用收到影响」*

相反，我们希望开发一个独立于具体类型结构的持久化组件。
...
<ins>更好的解决方案是定义一种易于修改和扩展的架构。
由此构建的系统可以按需适配不断变化的需求</ins>。
换言之，我们希望面向变化与演进进行设计。
该问题涉及多个因素 (forces)：
...
每一处变更都必须谨慎实施并进行测试。
<ins>能够主动支持并管控自身修改的软件，其变更过程会更高效、更安全</ins>。
*「要安全，不容易出错」*

- 可适配的软件系统通常具有复杂的内部结构。
易发生变化的方面被封装在独立的组件中。
应用服务的实现分布在许多具有不同相互关系的小型组件之上 [GHJV95](../ref.md#ghjv95) 。
<ins>为了保持此类系统的可维护性，我们倾向于向系统维护人员隐藏这种复杂性</ins>。
*「封装复杂性」*

- 维持系统可变性所需的技术越多，例如参数化、子类化、混入 (mix-ins)，甚至是复制粘贴，其修改就会变得越笨拙、越复杂。
<ins>一种适用于所有类型变更的统一机制会更易于使用和理解</ins>。
*「统一的修改机制」*

- 修改可以是任何规模的 (scale)，从为常用命令提供快捷方式，到为特定客户适配应用程序框架不等。
*「修改的目标差异大」*

- 即便是软件系统的基本 (fundamental) 方面也可能发生变化，例如组件之间的通信机制。
*「可以修改底层」*
...
<ins>元层提供软件的自我表示，使其具备关于自身结构和行为的认知，它由所谓的元对象 (metaobjects) 构成。
元对象封装并表示关于软件的信息，例如类型结构、算法，甚至函数调用机制</ins>。

<ins>基础层定义应用逻辑。
其实现通过使用元对象，来保持与那些可能发生修改的方面相互独立。
例如，基础层组件之间可以仅通过一个元对象进行通信，该元对象实现了特定的用户自定义函数调用机制。
修改此元对象即可改变基础层组件的通信方式，而无需修改基础层代码</ins>。

<ins>需要为操作元对象指定一种接口。
该接口被称为 *元对象协议（metaobject protocol, MOP）* ，它允许客户端指定特定的修改，例如修改上文提到的函数调用机制元对象</ins>。
元对象协议本身负责检查变更规范的正确性，并执行变更。
通过元对象协议对元对象执行的每个操控，都会影响后续基础层的行为，正如函数调用机制示例中所展示的那样。

↘️对于我们位于示例应用基础层的持久化组件，我们指定了提供运行时类型信息 (run-time type information) 的元对象。
例如，要存储一个对象，我们必须知道其内部结构以及所有数据成员的布局。
借助这些可用信息，我们可以递归遍历任意给定的对象结构，将其分解为一系列内置类型。
持久化组件 “知道” 如何存储这些类型。
如果我们修改运行时类型信息，也就修改了存储方法的行为。
例如，不再持久化的类的对象将不再被存储。
对每个方法采用类似的策略，我们就可以构建一个能够读取和存储任意数据结构的持久化组件。

## 结构 (Structure)
<ins>元层由一组元对象构成。
每个元对象都封装了关于基础层的结构、行为或状态中某一选定方面的信息</ins>。这类信息有三个来源：

- 它可以由系统的运行时环境提供，例如 C++ 类型标识对象 [DWP95](../ref.md#dwp95) 。
- 它可以是用户自定义的，例如上一节中的函数调用机制。
- 它可以在运行时从基础层获取，例如关于当前计算状态的信息。

<ins>所有元对象共同构成了应用的自表示（self-representation）。
...
并可以选择最高效的函数调用机制与之通信。
函数调用机制本身也可以由其他元对象提供。
更多示例包括：类型结构、实时约束、进程间通信机制以及事务协议 (transaction protocols)。

然而，<ins>使用元对象所表示的内容取决于需要具备适应性的部分</ins>。
只有那些可能发生变化或因客户而异的系统细节才应由元对象进行封装。
在应用程序整个生命周期中预期保持稳定的系统方面则不应如此。
*「只应针对部分应用内容使用元对象，按需而动」*

元对象的接口允许基础层访问其维护的信息或提供的服务。
例如，提供分布式组件位置信息的元对象会提供相应函数，用于访问组件的名称与标识符、组件所在进程的信息，以及进程运行所在主机的信息。
实现函数调用机制的元对象会提供一种方法，用于激活特定接受者的特定函数，包括输入与输出参数的传递。
<ins>元对象不允许基础层修改其内部状态，仅能通过元对象协议或其自身的计算来进行操控</ins>。

<ins>基础层使用元对象提供的信息和服务，例如组件的位置信息以及函数调用机制</ins>。
这使得基础层能够保持灵活性 —— 其代码独立于那些可能发生变更和适配的方面。
通过使用元对象的服务，基础层组件无需对通信伙伴的具体位置信息进行硬编码，它们会查询相应的元对象来获取此类信息。

<ins>基础层组件要么直接连接到其所依赖的元对象，要么通过专门的检索函数 (retrieval functions) 向这些元对象提交请求</ins>。
这些函数也属于元层的一部分。
如果基础层与元对象之间的关系相对静态，则优先采用第一种连接方式。
基础层组件始终查询同一个元对象，例如当某个对象需要自身的类型信息时。
如果基础层所使用的元对象会动态变化，则采用第二种连接方式，比如我们持久化组件中的存储过程场景。
*「第一种连接方式：直接连接元对象；第二种连接方式：通过检索函数发送请求」*

<img src="../img/fig-2522.png" width="90%"/></br>

<ins>元对象协议（MOP）作为元层的外部接口，使反射系统的实现以一种定义好的方式可访问</ins>。
元对象协议的客户可以是基础层组件、其他应用或有特权的用户，他们可以使用基础层指定对元对象或其关系的修改。
元对象协议本身负责执行这些更改。
这为反射应用提供了对自身修改的显式控制。
...
随后，元对象协议执行变更。
例如，它可以通过生成一个合适的元对象（其中包含用户为新机制定义的代码）、编译所生成的元对象、将其与应用动态链接，并将所有对 “旧” 元对象的引用更新为 “新” 元对象来完成这一过程。

<ins>元对象协议（MOP）通常被设计为一个独立的组件</ins>。
这支持对多个元对象进行操作的函数的实现。
例如，修改封装了分布式组件位置信息的元对象，最终需要更新对应的函数调用机制元对象。
如果将此类变更的职责委托给元对象自身，就很难维持它们之间的一致性。
元对象协议能够更好地控制所执行的每一次修改，因为它是独立于元对象实现的。
*「这里的一致性，是指两个元对象之间的逻辑一致性」*

<img src="../img/fig-2523.png" width="50%"/></br>

为执行变更，元对象协议需要访问元对象的内部结构。
如果它还有权更改基础层对象与元对象之间的连接 (connections)，它也需要访问基础层组件。
提供这种访问的一种方式是允许元对象协议直接操作它们的内部状态。
...
反射架构的整体结构与分层系统（[31](#todo)）非常相似。
元层和基础层是两个层次，各自提供自身的接口。
基础层规定了用于使用应用功能的用户接口，元层则定义了用于修改元对象的元对象协议。

<img src="../img/fig-2524.png" width="90%"/></br>

<ins>然而，与分层架构不同的是，这两层之间存在相互依赖关系</ins>。
基础层构建于元层之上，反之亦然。
后者的一个例子是，当异常发生时元对象实现的行为被执行。
必须执行的异常处理过程类型通常取决于当前的计算状态。
元层从基础层获取这些信息，通常来自提供被中断服务组件之外的不同组件。
在纯分层架构中，层与层之间的这种双向依赖是不允许的，每一层仅依赖于其下方的层。

↘️对我们的持久化组件示例来说，我们指定了元对象，这些元对象提供了对应用类型结构的自省访问 (introspective) —— 也就是说，它们可以访问有关应用结构或行为的信息，但不能对其进行修改。
我们可以获取给定类型或对象的名称、大小、数据成员以及父类信息。
另一个元对象规定了一个函数，该函数允许客户端实例化任意类型的对象。
例如，当从数据文件中恢复对象结构时，我们会使用此函数。
<ins>元对象协议包含用于添加新的运行时类型信息以及修改现有运行时类型信息的函数</ins>。

持久化组件的主体与应用程序的具体类型结构无关。
例如，（持久化组件）存储的过程仅实现通用算法，用于将给定的对象结构递归分解为一系列内置类型。
...
有关所涉及的元对象协议和元对象的详细信息，请参见 [实现部分](#实现-implementation) 。

### 场景一
<ins>展示了从磁盘文件中读取已存储对象时，基础层与元层之间的协作过程</ins>。
...
- `read()` 过程打开数据文件，并调用内部的 `readobject()` 过程，该过程读取第一个类型标识符 (type identifier) 。

- `readObject()` 过程调用负责对象创建的元对象。
该 “对象创建器 (object creator)” 元对象对先前确定的类型实例化一个 “空” 对象，
并返回该对象的句柄以及对应的运行时类型信息（run-time type information, RTTI）元对象的句柄。

- `readObject()` 过程向其对应的元对象请求一个迭代器，用于遍历待读取对象的数据成员。
该过程遍历对象的所有数据成员。

- `readObject()` 过程读取下一个数据成员的类型标识符。
如果该类型标识符表示内置类型（此情况图中没有展示），`readObject()` 过程会根据数据成员在对象中的大小和偏移量，直接将文件中的下一个数据项赋值给该数据成员。
否则，将递归调用 `readObject()`。
...
### 场景二
<ins>展示了在向元层添加类型信息时元对象协议的使用方法</ins>。
假设应用所使用的类库更新到了包含新类型的新版本。
为了能够存储和读取这些类型，我们必须通过新增元对象来扩展元层。
添加此类信息可由用户执行，或通过工具自动完成。
为简化起见，我们将 [实现部分](#实现-implementation) 中规定的 `type-info` 类与 `extTypeInfo` 类合并。
...
- 最后，客户端调用元对象协议来适配 “对象创建器 (object creator)” 元对象。
持久化组件在读取持久化数据时必须能够实例化该新类型的对象。
元对象协议基于此前添加的类型信息，自动生成用于创建新类型对象的代码。
它还将新生成的代码与 “对象创建器 (object creator)” 元对象的现有实现进行整合，编译修改后的实现，并将其与应用程序进行链接。
...
↘️我们 C++ 磁盘存储示例中的持久化组件是仓库管理应用程序的一部分 [Coad95](../ref.md#coad95) 。
...
### 2. 识别可变行为
<ins>分析上一步构建的模型，确定应用服务中哪些可能发生变化、哪些保持稳定</ins>。
目前没有通用规则来界定系统中哪些部分可以变更。
某个方面是否可变取决于诸多因素，例如应用领域、应用运行环境及其客户与用户。
在一个系统中可能变化的方面，在其他系统中可能保持稳定。
以下是通常会发生变化的系统方面的示例：

- 实时约束 [HT92](../ref.md#ht92) ，如截止时间 (deadlines)、时间围栏 (time-fence) 协议以及用于检测截止时间错过的算法。
- 事务协议 [SW95](../ref.md#sw95) ，例如会计系统中的乐观与悲观事务控制。
- 进程间通信机制 [CM93](../ref.md#cm93) ，如远程过程调用和共享内存。
- 异常情况下的行为 [EKM+94](../ref.md#ekm94) 、[HT92](../ref.md#ht92) ，例如实时系统中对截止时间错过的处理。
- 应用服务的算法 [EKM+94](../ref.md#ekm94) ，如特定国家的增值税 (VAT) 计算。

开放实现分析与设计方法 (The Open Implementation Analysis and Design Method) [KLLM95](../ref.md#kllm95) 有助于此步骤。

↘️为保持持久化组件示例简化，我们不考虑对应用行为进行适配。

### 3. 识别系统的结构方面
识别系统的结构方面，这些方面在发生变化时不应影响基础层的实现。
示例包括应用的类型结构 (type structure) [BKSP92](../ref.md#bksp92) 、其底层对象模型 [McA95](../ref.md#mca95) ，或是异构网络中组件的分布 [McA95](../ref.md#mca95) 。

↘️我们对持久化组件的实现必须独立于具体应用的类型。
这就要求能够访问运行时类型信息，例如每种类型的名称、大小、继承关系和内部布局，以及其数据成员的类型、顺序和名称。

### 4. 识别系统服务 
识别那些既能支持 [步骤 2](#2-识别可变行为) 中所确定的应用服务变体，又能支持 [步骤 3](#3-识别系统的结构方面) 中所确定的结构细节独立性。
...
↘️持久化组件在读取持久化对象时，必须能够实例化任意类。

### 5. 定义元对象
<ins>针对前面三个步骤所确定的每个方面，定义合适的元对象</ins>。
若干与领域无关的设计模式可支持行为封装，例如对象化模式（Objectifier）[Zim94](../ref.md#zim94) 、策略模式（Strategy）、桥接模式（Bridge）、访问者模式（Visitor）以及抽象工厂模式（Abstract Factory）[GHJV95](../ref.md#ghjv95) 。
例如，函数调用机制的元对象可作为策略对象实现，而组件的多种实现可采用桥接模式实现。
访问者模式允许在不修改现有结构的前提下集成新功能。
有时你还可以找到合适的领域特定模式来支持此步骤，例如用于开发分布式系统的接收器模式（Acceptor）与连接器模式（Connector）[Sch95](../ref.md#sch95) 。
另一个例子是可分离检查器模式（Detachable Inspector）[SC95a](../ref.md#sc95a)，它支持添加调试器和检查器等运行时工具。
可分离检查器模式（Detachable Inspector）基于访问者模式（Visitor）构建。
对象化模式（Objectifier）[Zim94](../ref.md#zim94) 和状态模式（State）[GHJV95](../ref.md#ghjv95) 等设计模式可支持结构与状态信息的封装。
*「喔，这么多模式可用于定义元对象」*

↘️为我们的持久化组件提供运行时类型信息的元对象，其组织方式如下：

<ins>C++ 标准库中的类 `type-info` 用于标识类型 [DWP95](../ref.md#dwp95) 。
其接口提供了用于访问类型名称、比较两种类型，以及确定它们在系统内部排序的函数。
应用程序中的每一种类型都由 `type-info` 类的一个实例来表示</ins>。
...
<ins>运行时类型信息系统的其他类均不属于 C++ 标准的一部分</ins>。

<ins>`extTypeInfo` 类提供对类的大小、父类以及数据成员信息的访问。
客户端还可以判断该类型是内置类型还是指针类型</ins>。
...
<ins>`BaseInfo` 类提供用于访问某个类的基类的类型信息，以及确定该基类在类布局中的偏移量的函数</ins>。
*「这个类是上面 `extTypeInfo::bases()` 返回的基类」*
...
<ins>`DataInfo` 类包含用于返回数据成员名称、其偏移量以及关联的 `type_info` 对象的函数</ins>。
*「这个类是上面 `extTypeInfo::data()` 返回的类」*
...
<ins>后一种方案的优势在于，对反射应用的所有修改控制都集中在一个中心点</ins>。
对多个元对象进行操作的函数更容易实现。
此外，如果独立组件的实现遵循外观模式（Facade）[GHJV95](../ref.md#ghjv95) 或整体-部分模式（Whole-Part）([225](#todo)) 等模式，该组件还可以保护元对象免受未授权访问和修改。
单例用法（Singleton idiom）[GHJV95](../ref.md#ghjv95) 有助于确保元对象协议只能被实例化一次。

如果作为独立组件实现，元对象协议通常不会作为定义元对象的类的基类 —— 它仅对这些元对象进行操作。
只有当元对象协议适用于所有元对象时，将其指定为派生具体元对象类的基类才有意义。

↘️<ins>我们提供一个 `MOP` 类，它为我们持久化组件示例中的元层定义了元对象协议。
该类以单例模式实现，并直接操作上一步中声明的所有类的内部结构</ins>。
...
<ins>可以通过两个函数初始化新的类型信息元对象，一个用于实例化 `type_info` 对象，另一个用于创建 `extTypeInfo` 对象</ins>。
...
<ins>我们定义了四个函数用于添加新的类型信息或修改已有的类型信息</ins>。
...
<ins>在内部，元对象协议需要一些函数来计算类型大小、基类偏移量以及数据成员偏移量。
这些函数与编译器相关，因此在使用不同编译器时必须进行修改</ins>。
策略模式（Strategy）[GHJV95](../ref.md#ghjv95) 提供了一种支持修改这些函数的方法。
...
<ins>实现元对象协议时，健壮性 (robustness) 是一个主要关注点。
应尽可能检测修改规范中的错误，修改也应具备可靠性</ins>。
例如，上述元对象协议在添加新的基类和数据成员信息时，会检查相应类型信息元对象是否可用；
在删除其类型信息之前，还会检查某个类型是否被用作基类或数据成员。

<ins>健壮性 (robustness) 还意味着保持一致性。
例如，如果我们为某个特定类型添加数据成员，就必须重新计算所有将该变更类型作为基类或数据成员的类型的大小</ins>。
...
<ins>将每个基础层组件与元对象相连，元对象提供它们所依赖的系统信息，如类型信息，或提供它们所需的服务，如持久化组件中的对象创建</ins>。
为处理 (handle) 系统服务，可采用设计模式，如策略模式（Strategy）、访问者模式（Visitor）、抽象工厂模式（Abstract Factory）和桥接模式（Bridge）[GHJV95](../ref.md#ghjv95) ，或诸如信封-字母（Envelope-Letter）[Cope92](../ref.md#cope92) 之类的惯用法 (idioms) 。
例如，策略模式（Strategy）中的上下文类组件代表基础层组件，而策略类层次结构则对应元对象。
应用访问者模式（Visitor）时，元对象即为访问者，对象结构则代表基础层组件。

为基础层组件提供函数，用于维护其与关联元对象之间关系。
<ins>元对象协议必须能够修改基础层与元层之间的每一种关系</ins>。
...
<ins>元对象通常需要有关当前计算状态的信息</ins>。
例如，我们持久化组件示例中的 “对象创建器” 必须知道它应实例化的类型。
此类信息既可以作为参数传递给元对象，也可以由元对象从其他元对象中获取，或者由元对象从相应的基础层组件中获取。

<ins>对元对象的修改会影响与其相连的基础层组件的后续行为</ins>。
改变基础层与元层之间的某一关系，仅会影响特定基础层组件，其维护被修改的关系。

↘️我们持久化组件的 `read()` 方法实现遵循 [动态部分](#动态-dynamics) 所描述的第一种场景。
...
为实现这一点，在存储对象前，该方法会用一个唯一标识符标记对象结构，且将该标识符也会被存储。
...
void Persistence::storeObject (void* object, char* typeName) {
  ...
  // Object is a user-defined type with data members
      ...
      if (!marked((char*)object + iterator->curr()->offset()))
...
一种具有多个元层的编程语言示例是 RbCl [IMY92](../ref.md#imy92) 。
RbCl 是一门解释型语言。
...
### CLOS
这是反射式编程语言的经典范例 [Kee89](../ref.md#kee89) 。
在CLOS中，为对象定义的操作被称为 *泛化函数 (generic functions)* ，其处理过程称为 *泛化函数调用 (generic function invocation)* 。
泛化函数调用分为三个阶段：
...
该层支持开发灵活的进程间通信机制，或开发诸如检视器 (inspector) 这类需要应用的类型结构详细信息的工具。
...
### PGen
PGen [THP94](../ref.md#thp94) 是一个基于 MIP 的 C++ 持久化组件。
它允许应用程序存储和读取任意的 C++ 对象结构。

<ins>用于解释反射模式的示例主要基于 MIP 和 PGen</ins>。
尽管经过简化，但对持久化组件、元对象的类声明以及元对象协议的描述，在很大程度上反映了 MIP 和 PGen 的原始结构。
*「本节介绍中的示例来源」*

### NEDIS
汽车经销商系统 NEDIS [Ste95](../ref.md#ste95) 利用反射来支持其适配客户与国家特定需求的能力。
...
### OLE 2.0
OLE (Object Linking and Embedding) 2.0 [Bro94](../ref.md#bro94) 提供了用于暴露和访问 OLE 对象及其接口的类型信息的功能。
该信息可用于动态访问 OLE 对象的结构信息，并创建对 OLE 接口的调用。
例如，Visual Basic [Mic95](../ref.md#mic95) 的运行时环境在动态调用对象之前会检查方法调用的正确性。
CORBA [OMG92](../ref.md#omg92) 中也规定了类似的概念。

其他采用反射架构的语言与系统实例还包括 Open C++ [CM93](../ref.md#cm93)、RbCl [IMY92](../ref.md#imy92)、AL-1/D [OIT92](../ref.md#oit92)、R2 [HT92](../ref.md#ht92)、Apertos [Yok92](../ref.md#yok92) 以及 CodA [McA95](../ref.md#mca95)。
更多实例可参见 [IMSA92](../ref.md#imsa92)，但需要注意的是，尽管所有这些实例都提供了反射功能，但并非全部都真正实现了本模式所描述的反射架构。
...
元对象协议还会管控每一次修改。
设计良好且健壮的元对象协议，有助于避免对应用程序基本语义造成非预期的修改 [Kic92](../ref.md#kic92) 。
...
<ins>然而，反射架构也存在一些显著的 **弊端** ：</ins>

<ins>*元层的修改可能会造成破坏*</ins>。
...
因此，元对象协议的健壮性至关重要 [Kic92](../ref.md#kic92) 。
...
不过，即使在不具备反射能力的语言中，也可以构建可修改、可扩展的反射系统，例如 C++ 系统 NEDIS [EKM+94](../ref.md#ekm94) 、MIP [BKSP92](../ref.md#bksp92) 和 Open C++ [CM93](../ref.md#cm93) 。

## 参见 (See Also)
微内核架构模式（ [171](5-1.md) ）通过提供一种机制来支持适配与变更，该机制可利用附加功能或客户特定功能对软件进行扩展。
此架构的核心组件 ——微内核—— 充当插槽 (socket) 的角色，用于插入此类扩展并协调它们之间的协作。
可通过替换这些 “可插拔 (pluggable)” 部件来实现修改。

该模式的早期版本发表于 [PLoP95](../ref.md#plop95) 。

## 致谢 (Credits)
关于反射的早期研究成果之一是 Brian Cantwell Smith 的博士论文 [Smi82](../ref.md#smi82) ，该论文在过程式语言的背景下阐述了反射理论。
反射概念的综述可参阅 [Mae87](../ref.md#mae87) 。
...
这部分代码，是 AI 生成的，放在这里仅做参考，待后续找到合适的 Java 示例代码，再更新。
...
// ======================
// 1. 元对象协议 MOP（核心！POSA1 定义的控制层）
// ======================
class MetaObjectProtocol {
    // 单例（POSA1 原文明确要求 Singleton）
    private static MetaObjectProtocol instance = new MetaObjectProtocol();
    private MetaObjectProtocol() {}
    public static MetaObjectProtocol getInstance() {
        return instance;
    }

    // 缓存：类 -> 元对象（typeInfo + extTypeInfo，对应你翻译的内容）
    private final Map<Class<?>, MetaObject> metaMap = new HashMap<>();

    // 获取元对象
    public MetaObject getMetaObject(Class<?> clazz) {
        if (!metaMap.containsKey(clazz)) {
            metaMap.put(clazz, new MetaObject(clazz));
        }
        return metaMap.get(clazz);
    }

    // 动态创建对象（POSA1：object creator）
...
// ======================
// 2. 元对象（Meta Object）：描述基础对象的结构
// ======================
class MetaObject {
    private final Class<?> clazz;

    public MetaObject(Class<?> clazz) {
        this.clazz = clazz;
    }

    // 打印类结构（对应 extTypeInfo）
    public void showStructure() {
        System.out.println("=== 元信息（类结构）===");
...
// 测试：运行反射模式
// ======================
public class ReflectionPatternDemo {
    public static void main(String[] args) throws Exception {
        // 1. 获取 MOP（单例）
        MetaObjectProtocol mop = MetaObjectProtocol.getInstance();

        // 2. 动态创建对象（无需 new User()）
        User user = (User) mop.newInstance(User.class);

        // 3. 动态设置值
        mop.setField(user, "name", "POSA1-Reflection");
        mop.setField(user, "age", 18);

        // 4. 动态获取值
        System.out.println("动态获取：name = " + mop.getField(user, "name"));
        System.out.println("对象：" + user);

        // 5. 获取元对象，查看类结构
        MetaObject meta = mop.getMetaObject(User.class);
        meta.showStructure();
    }
}
```
...
```txt
动态获取：name = POSA1-Reflection
对象：User{name='POSA1-Reflection', age=18}
=== 元信息（类结构）===
类名：User
父类：java.lang.Object
字段：
