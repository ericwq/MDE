...
在高层次上，设计模式是应用于软件的 “从蓝图构建” 选项。
就像棚子的蓝图包含尺寸、门窗位置和屋顶材料一样，设计模式也包含一些关于我们编写的代码的规范和细节。
...
我们通常将接口的这个方面称为其灵活性，并称用户能够轻松适应变化的接口为灵活的，而即使是微小的更改（如重命名字段）也会导致完全失败的接口为僵化的。
这种区分很重要，因为进行大量更改的能力在很大程度上取决于接口的灵活性。
最重要的是，我们可以看到僵化的接口使我们更难像在其他软件项目中那样迭代地迈向出色的设计。
这意味着我们最终往往会被所有设计决策所困，无论好坏。
这可能会让你认为 API 的僵化意味着我们将永远无法使用迭代开发过程，但情况并非总是如此，这要归功于接口的另一个重要方面：可见性。
...
那么，为什么 API 是特殊的呢？
<ins>事实证明，当我们设计许多 API（其本身是僵化的）并与全世界分享时，我们在这两个方面确实面临着最坏的情况。
这意味着进行更改比这两种属性的任何其他组合都要困难得多（如表 2.2 所总结）。</ins>

*表 2.2 更改各种接口的难度* <br/>
<img src="./img/tab-2-2.png" width="90%"><br/>

简而言之，这种 “两全其坏” 的场景（既僵化又难以更改）使得可重用且经过验证的设计模式对于构建 API 来说，比其他类型的软件更为重要。
<ins>在大多数软件项目中，代码通常是私有的且不为人所见，而 API 中的设计决策则处于前沿和中心位置，展示给服务的所有用户。
由于这严重限制了我们对设计进行渐进式改进的能力，因此依赖那些经受住时间考验的现有模式，对于在第一次就做对事情非常有价值，而不是像大多数软件那样最终做对。</ins>
...
目录中的每个设计模式都有一个名称，用于在目录中唯一标识该模式。
...
由于 API 设计模式的目标是为某一类问题提供解决方案，因此一个好的起点是定义该模式旨在覆盖的问题空间。
本节旨在解释基本问题，以便轻松理解为什么我们首先需要为此建立一个模式。
...
现在我们已经接近有趣的部分了：解释设计模式推荐作为问题空间解决方案的内容。
在这一点上，我们不再关注定义问题，而是提供解决方案的高层描述。
...
在这一点上，我们理解了设计模式给了我们什么，但我们还没有讨论它带走了什么，这实际上非常重要。
...
随着越来越多的人使用该服务，这个消息列表可能会变得相当长。
当响应开始包含数十或数百条消息时，这可能不是什么大问题，但当你开始达到数千、数十万甚至数百万条时呢？
...
为此，我们可以依赖分页模式（参见 [第 26 章](ch26.md) ）。

#### 分页模式

正如我们将在 [第 26 章](ch26.md) 中了解到的，分页模式是一种以更小、更易于管理的数据块（而非一次性发送整个列表）来检索长列表项的方式。
...
interface ExportMessagesResponse {
    // The location of a compressed file containing the messages requested.
    // Twapi服务返回压缩文件的地址，后续可从文件服务器下载该文件，而非直接从此API服务获取
    exportDownloadUri: string;
}
```

这个 API 确实完成了主要任务（导出数据）和几个次要任务（异步检索），但遗漏了一些重要方面。
...
正如我们将在 [第 28 章](ch28.md) 中了解到的，导入/导出模式正是针对这样的问题：我们在 API 服务中有一些数据，消费者希望有一种方法将其取出（或放入）。
然而，与我们之前讨论的分页模式不同，这种模式将依赖于其他模式，例如长时间运行的操作模式（在 [第 13 章](ch13.md) 中讨论），来完成工作。
...
abstract class Twapi {
    ...
    // 和前面示例不同，ExportMessages方法返回类型为长时运行操作，操作完成返回ExportMessagesResponse，通过ExportMessagesMetadata接口上报操作元数据
    @post("/{parent=users/*}/messages:export")
...
我们将在 [第 28 章](ch28.md) 中更详细地探讨这个模式，但就目前而言，让我们先抛出这些接口的一个简单 API 定义，以便我们至少对它们正在做什么有一个高层次的理解。
...
- API 设计模式之所以重要，是因为 API 通常非常 “僵化”，因此不易更改，所以设计模式有助于最大限度地减少对大型结构性更改的需求。
