...
## 启动 LM Studio
 ...
 [LM Studio](https://lmstudio.ai/) 将提供 LLM 及推理引擎。你需要预先下载相应的 LLM 文件，这个例子中使用的是：qwen3.6-35b-a3b。
...
- 运行 pi.dev `nono run -v --profile pi-dev pi` 
  - -v 选项，打印所有授权内容 
  - 此时你可以检查授权了对哪些资源的访问
- 将 [pi-dev-models.json](./pi-dev-models.json) 另存为文件 `~/.pi/agent/models.json` 
  - 文件名更改为 `models.json`，
  - `models.json` 中已经预置了上一节中提到的模型，你可以查看一下这个文件，
